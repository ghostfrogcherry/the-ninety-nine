 ...
 *
 *  - `booster` and `set_type` (0008_drafts.sql) are what lib/draft uses to
 *    decide which printings go in a pack and which sets are draftable. Absent
 *    stays NULL, never `false`: lib/draft reads a set with no known `booster`
 *    as "unknown, use every row", and a `false` coerced out of a missing key
 *    would instead make the set look like it has no booster cards at all.
 */
...
export function toCardRow(card) {
  ...
  return [
    ...
    // Only a real boolean counts as known. pg would happily coerce "t" or
    // "yes" into TRUE, and a guess here decides what goes in every pack.
    typeof card.booster === "boolean" ? card.booster : null,
    nullIfBlank(typeof card.set_type === "string" ? card.set_type : null),
  ];
}
